...
class Plotter(object):
    ...
    def plot_decision_regions(features, target, clf, test_idx=None, title=None):
        ...
        if test_idx:
            # plot all samples
            features_test, target_test = features[test_idx,
                                                  :], target[test_idx]
            plt.scatter(
                features_test[:, 0],
                features_test[:, 1],
...
class Evaluator(object):
    ...
    def k_fold_cv(
        ...
        for __clf in __clfs:
            ...
            for k, (train, test) in enumerate(kf):
                ...
                s_f1 += f1_score(
                    target.iloc[test], target_pred.round(), average="macro") * 100
    ...
    def run_rbf_svm(
        ...
        print(
            "SVM hyper-parameters: C={}; gamma={}".format(
...
def main():
    ...
    linear_params, rbf_params = Evaluator.find_best_params(
        features_train, target_train, n_folds=10, scoring=scorer)
    # train and test model trained on K-fold cross validation
    ev.k_fold_cv(features, target, n_splits=10,
                 linear_params=linear_params, rbf_params=rbf_params)
    # train and test linear SVM model with best parameter
    ev.run_linear_svm(features_train, features_test,
                      target_train, target_test, params=linear_params)
    # train and test rbf SVM model with best parameter
    ev.run_rbf_svm(features_train, features_test, target_train,
                   target_test, params=rbf_params)
